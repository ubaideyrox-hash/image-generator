...
exports.newPlaylist = function (playlist, cb) {
    var file = path.join(config.mediaDir, ("__" + playlist['file'] + '.json')),
        data = {
            name: playlist['file'], description: playlist['description'], user_id: playlist['user_id'], createdAt: Date.now(), settings: {
            ...
            var readFile = function (plfile, cb) {
                ...
                fs.readFile(path.join(assetDir, plfile), 'utf8', function (err, data) {
                    if (err || !data)
                        ...
                        playlist.createdAt = obj.createdAt;

                        if(playlist.user_id==req.params.user_id){
                            list.push(playlist);
                        }
                    }
                    cb();
...
exports.createPlaylist = function (req, res) {
    ...
    exports.newPlaylist(req.body, function (err, data) {
        if (err) {
        ...
        if (err) {
            ...
            if (data) {
                try {
                    ...
                    console.log("savePlaylist parsing error for ",e)
                }
